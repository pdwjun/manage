 ...
 * @var \vova07\roles\models\backend\User $model Model
 * @var \vova07\roles\models\Profile $profile Profile
    ...
    <div class="row">
        <div class="col-sm-6">
            ...
            $form->field($user, 'status_id')->dropDownList(
                $statusArray,
                [
                    'prompt' => Module::t('roles', 'BACKEND_PROMPT_STATUS')
                ]
            ) ?>
        </div>
        <div class="col-sm-6">
            ...
            $form->field($user, 'role')->dropDownList(
                $roleArray,
                [
                    'prompt' => Module::t('roles', 'BACKEND_PROMPT_ROLE')
                ]
            ) ?>
        </div>
    </div>
...
<?php $box->beginFooter(); ?>
<?= Html::submitButton(
    $user->isNewRecord ? Module::t('roles', 'BACKEND_CREATE_SUBMIT') : Module::t('roles', 'BACKEND_UPDATE_SUBMIT'),
    [
